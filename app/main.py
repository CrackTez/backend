...
IPFS_PROVIDERS = ["cloudflare-ipfs.com", "cf-ipfs.com", "gateway.ipfs.io",
                  "ipfs.infura.io", "ipfs.io"]
...
def get_post_by_id():
    ...
    cur.execute(
        f'''SELECT idKey, txnHash, title, author, contentHash, thumbnailHash, fundraising_goal, fundraised FROM {table} WHERE idKey = "{id}"''')
...
def get_posts_by_author():
    ...
    cur.execute(
        f'''SELECT idKey, txnHash, title, author, contentHash, thumbnailHash, fundraising_goal, fundraised FROM {table} WHERE author = "{author}"''')
